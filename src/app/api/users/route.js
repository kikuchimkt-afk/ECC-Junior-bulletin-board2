...
// 初期ユーザー（KVが空の場合に使用）
const DEFAULT_USERS = {
    'user001': { id: 'user001', password: 'pass001', name: '田中 花子', email: '', isAdmin: false, isTeacher: false, schools: [] },
    'user002': { id: 'user002', password: 'pass002', name: '鈴木 太郎', email: '', isAdmin: false, isTeacher: false, schools: [] },
    'admin': { id: 'admin', password: 'adminpass', name: '管理者', email: '', isAdmin: true, isTeacher: false, schools: [] }
};
...
export async function POST(request) {
    try {
        ...
        // ユーザー保存
        const user = { id, password, name, email: body.email || '', isAdmin: !!isAdmin, isTeacher: !!body.isTeacher, schools: body.schools || [] };
        await kv.hset('users', { [id]: JSON.stringify(user) });

        return NextResponse.json({
            success: true,
            user: { id, name, email: user.email, isAdmin: !!isAdmin, isTeacher: !!body.isTeacher, schools: user.schools }
        });
    } catch (error) {
